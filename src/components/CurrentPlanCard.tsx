...
interface CurrentPlanCardProps {
  ...
  onViewPlans: () => void;
  isDark?: boolean;
  planType?: 'free' | 'basic' | 'premium' | 'ultimate';  // New prop for plan type
}
...
const CurrentPlanCard = ({
  ...
  const isPremiumUser = planType === 'premium' || planType === 'basic';
    ...
    <Card className={`${isDark ? 'glass-card border-slate-700' : 'bg-white/90 border-slate-200'} mb-4 mx-4`}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          >
            {isFreeUser ? 'Free Plan' : isUltimateUser ? 'Ultimate Plan' : 'Premium Plan'}
          </Badge>
        </div>
      </CardHeader>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              {isFreeUser
                ...
                : isUltimateUser
                ? 'Unlimited'
                : `${interpretationsLeft} interpretations`}
            </span>
          </div>
